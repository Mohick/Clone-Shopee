...
function ItemsFlashSaleMainPage() {
  ...
  useEffect(() => {
    let allow = true;
    axios
      .get("https://run.mocky.io/v3/6c32526e-0ea9-46fc-a83f-791cc8aa4426")
      .then((response) => {
        if (allow) {
  ...
  return (
    <div className="flash__sale--body">
        ...
        return (
          <div className="flash__sale__body--items" key={index}>
            ...
            <div className="flash__sale__body__items--cost">
              ...
              <div className="flash__sale__body__items--cost--price">{items.cost}</div>
            </div>
            <div className="flash__sale__body__items--sell">
              {(items.sold/items.sale)*100 == 100 ? (
                <div className="flash__sale__body__items__sell--sold">
                  <div className="flash__sale__body__items__sell__sold--title"> sold out </div>
                  <div className="flash__sale__body__items__sell__sold--progress">
                  </div>
                </div>
              ) : (
                <div className="flash__sale__body__items__sell--still">
                  <div className="flash__sale__body__items__sell__still--title">{items.sold} Sold</div>
                  <div className="flash__sale__body__items__sell__still--progress" style={{width:`${(items.sold/items.sale)*100}%`}}></div>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="flash__sale__body--arrow--left  " onClick={()=>{
        handleArrowFlashSale('left')
      }}>
        <IconAngleLeft width={'2rem'} height={'2rem'}/>
      </div>
      <div className="flash__sale__body--arrow--right flash__sale__body__arrow--active" onClick={()=>{
